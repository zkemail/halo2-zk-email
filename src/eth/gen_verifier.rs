...
pub fn gen_sol_verifiers(params: &ParamsKZG<Bn256>, vk: &VerifyingKey<G1Affine>, max_line_size_per_file: usize, sols_dir: &PathBuf) {
    ...
    // }
    fs::create_dir_all(&sols_dir).unwrap();
    let sols_gen = gen_evm_verifier_from_vk(params, vk, 3);
    ...
    let interface_sol = include_str!("./IVerifier.sol");
    fs::write(sols_dir.join("IVerifier.sol"), interface_sol).unwrap();
...
pub fn gen_evm_verifier_from_vk<'a>(params: &ParamsKZG<Bn256>, vk: &'a VerifyingKey<G1Affine>, num_instance: usize) -> halo2_solidity_verifier::SolidityGenerator<'a, G1Affine> {
    halo2_solidity_verifier::SolidityGenerator::new(params, vk, num_instance, None, halo2_solidity_verifier::BatchOpenScheme::Bdfg21)
}
