...
//     println!("address {:?}", verifier_addr);
//     let anvil = setup_eth_backend().await;
//     let client = setup_eth_client(&anvil).await;
//     let verifier = EmailVerifier::new(verifier_addr, client.clone());
...
//     let mut tx: TypedTransaction = TypedTransaction::default();
//     tx.set_to(verifier_addr);
//     tx.set_from(client.address());
//     tx.set_data(Bytes::from(encoded));
//     println!("tx {:?}", tx);
//     let result = client.call(&tx, None).await;
//     println!("result {:?}", result.unwrap());
//     drop(anvil);
//     // let call = verifier.verify_email(instance.clone(), proofs.clone());
//     // println!("call {:?}", call);
//     // let called = call.call().await;
//     // println!("called {:?}", called);
//     // let result = called.unwrap();
//     // let result = verifier.verify_email(instance.clone(), proofs.clone()).call().await.unwrap();
//     // println!("result {}", result);
//     // let call = verifier.method::<_, ()>("verify_email", (instance.clone(), proofs.clone())).unwrap().legacy();
//     // println!("estimated gas {:?}", call.estimate_gas().await.unwrap());
//     // println!("result 1 {:?}", call.call().await.unwrap());
//     // println!("abi {:?}", verifier.abi());
//     // let call = verifier.method::<_, ()>("verifyEmail", (instance, proofs)).unwrap();
//     // println!("raw bytes {:?}", call.call_raw_bytes().await);
//     // println!("estimated gas {:?}", call.estimate_gas().await.unwrap());
//     // // println!("call {:?}", call);
//     // println!("result 2 {:?}", call.call().await.unwrap());
//     // drop(anvil);
// }
...
pub async fn deploy_and_call_verifiers(sols_dir: &PathBuf, runs: Option<usize>, proofs: &[&[u8]], instance: &EmailVerifyInstancesJson) {
    // let mut evm = EVM {
    //     env: Default::default(),
...
// async fn deploy_verifier_base_and_funcs(evm: &mut EVM<InMemoryDB>, sols_dir: &PathBuf, dir_name: &str, runs: usize) -> (B160, u64) {
...
//         let (func_addr, gas) = deploy_verifier_via_solidity(evm, dir.join(format!("VerifierFunc{}.sol", idx)), &format!("VerifierFunc{}", idx), (), Some(runs)).await;
//         gas_sum += gas;
//         func_addrs.push(Address::from(func_addr));
//     }
//     let base_args = (func_addrs, U256::from(deploy_params.max_transcript_addr));
//     let (base_addr, gas) = deploy_verifier_via_solidity(evm, sols_dir.join("VerifierBase.sol"), "VerifierBase", base_args, Some(runs)).await;
//     gas_sum += gas;
//     (base_addr, gas_sum)
// }

async fn deploy_verifier_base_and_funcs(client: &EthersClient, sols_dir: &PathBuf, dir_name: &str, runs: usize) -> (Address, U256) {
    let dir = sols_dir.join(&dir_name);
    let deploy_params = serde_json::from_reader::<_, DeployParamsJson>(File::open(&dir.join("deploy_params.json")).expect(&format!("deploy_params.json in {:?} cannot open", dir)))
        .expect(&format!("fail to convert deploy_params.json in {:?}", dir));
    let mut func_addrs = vec![];
    let mut gas_sum = U256::zero();
    for idx in 0..deploy_params.num_func_contracts {
        let (func_addr, gas) = deploy_verifier_via_solidity(client, dir.join(format!("VerifierFunc{}.sol", idx)), &format!("VerifierFunc{}", idx), (), Some(runs)).await;
        gas_sum += gas;
        func_addrs.push(Address::from(func_addr));
    }
    let base_args = (func_addrs, U256::from(deploy_params.max_transcript_addr));
